...
all_mr = function(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval, method){
...
### "GSMR_heidi", "GSMR_noheidi", "IVW", "Median", "Mode", "Egger", "Robust", "Lasso", "RAPS", "PRESSO", "MRMix", "Con-Mix"
    library(MRMix);library(MendelianRandomization)
...
n1a=10000
    if(method=="GSMR_heidi_v1"){
     #library(gsmr)   
source("/home/uqaxue/90days/uqaxue/proj/addiction/NHB_revision_05AUG2022/simulation/gsmr_published_version.R")
snp_coeff_id = names(bzx); ldrho=diag(length(bzx)); colnames(ldrho) = rownames(ldrho) = snp_coeff_id
        mr_results = tryCatch(gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval,
                        ldrho=ldrho, snpid=snp_coeff_id, n_ref=n1a, heidi_outlier_flag=T,gwas_thresh=5e-8, heidi_outlier_thresh=0.01, nsnps_thresh=5),
  ...
  envir <- new.env()
  source('/home/uqaxue/90days/uqaxue/proj/addiction/NHB_revision_05AUG2022/simulation/gsmr_original_global.R')
  snp_coeff_id = names(bzx); ldrho=diag(length(bzx)); colnames(ldrho) = rownames(ldrho) = snp_coeff_id
  mr_results = tryCatch(gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval,
                             ldrho=ldrho, snpid=snp_coeff_id, n_ref=n1a, heidi_outlier_flag=T,gsmr2_beta=1,single_snp_heidi_thresh=0.01, multi_snps_heidi_thresh=0.01,nsnps_thresh=5),
                        error=function(e){list(bxy_pval=NA, bxy=NA,se=NA,pleio=NA)})
	return(list(pval=mr_results$bxy_pval, bxy=mr_results$bxy,se=mr_results$bxy_se,pleio=mr_results$pleio_snps))
   }else if(method=="GSMR_heidi_v3"){
  envir <- new.env()
  source('/home/uqaxue/90days/uqaxue/proj/addiction/NHB_revision_05AUG2022/simulation/gsmr_direct_global.R')
  snp_coeff_id = names(bzx); ldrho=diag(length(bzx)); colnames(ldrho) = rownames(ldrho) = snp_coeff_id
  mr_results = tryCatch(gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval,
                             ldrho=ldrho, snpid=snp_coeff_id, n_ref=n1a, heidi_outlier_flag=T,gsmr2_beta=1,single_snp_heidi_thresh=0.01, multi_snps_heidi_thresh=0.01,nsnps_thresh=5),
                        error=function(e){list(bxy_pval=NA, bxy=NA,se=NA,pleio=NA)})
  ...
  envir <- new.env()
  source('/home/uqaxue/90days/uqaxue/proj/addiction/NHB_revision_05AUG2022/simulation/gsmr_direct_global_stepwise.R')
  snp_coeff_id = names(bzx); ldrho=diag(length(bzx)); colnames(ldrho) = rownames(ldrho) = snp_coeff_id
  mr_results = tryCatch(gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval,
                             ldrho=ldrho, snpid=snp_coeff_id, n_ref=n1a, heidi_outlier_flag=T,gsmr2_beta=1,single_snp_heidi_thresh=0.01, multi_snps_heidi_thresh=0.01,nsnps_thresh=5),
                        error=function(e){list(bxy_pval=NA, bxy=NA,se=NA,pleio=NA)})
	...
	#library(gsmr)
        ...
        mr_results = tryCatch(gsmr(bzx, bzx_se, bzx_pval, bzy, bzy_se, bzy_pval,gwas_thresh=5e-8,
                        ldrho=ldrho, snpid=snp_coeff_id, n_ref=n1a, heidi_outlier_flag=F, nsnps_thresh=5),
	...
	return(list(pval=mr_results$bxy_pval, bxy=mr_results$bxy,se=mr_results$bxy_se))
        ...
        mr_results = mr_egger(mr_input(bx=bzx, bxse=bzx_se, by=bzy, byse=bzy_se))
        ...
        if(class(mr_results)!="try-error"){
            return(list(bxy = mr_results$ThetaEstimate, 
			se = mr_results$ThetaSE,
			pval = 2 * pnorm(abs(mr_results$ThetaEstimate/mr_results$ThetaSE), lower.tail = F)))    ### no p value provided
        }else{
        ...
        if(show_condition(MRMix(betahat_x=bzx, betahat_y=bzy, sx=bzx_se, sy=bzy_se, 
                        theta_temp_vec = seq(-0.5,0.5,by=0.01)))=="warning"){
	...
	return(list(pval=mr_results$pvalue_theta, bxy=mr_results$theta,se=mr_results$SE_theta))
        ...
        mr_results = mr_conmix(mr_input(bx=bzx, bxse=bzx_se, by=bzy, byse=bzy_se), CIMin = -0.5,CIMax = 0.5, CIStep = 0.01)
        b=mr_results$Estimate; se = (mr_results$CIUpper-mr_results$CILower)/2/1.96
	p=pchisq((b/se)^2,1,lower.tail=F)
	return(list(pval=mr_results$Pvalue[1], bxy=mr_results$Estimate[1],se=se[1]))   ### No pvalue in the results of mr_conmix() function
